function qrrt_star
    ...
    if goal_reached
        ...
        interpolated_path = interpolate_path(path, 1000);
        disp('Interpolated path stored in "interpolated_path".');
        highlight_path(interpolated_path);
    else
...
function path = generate_path(tree, goal)
    % Generate path from tree to goal
    path = [];
    current = size(tree.vertices, 1);
    while current > 0
        path = [tree.vertices(current, :); path];
        if isequal(tree.vertices(current, :), goal)
            break;
        end
        current_edge = find(tree.edges(:, 2) == current);
        if ~isempty(current_edge)
            current = tree.edges(current_edge, 1);
        else
            break;
        end
    end
...
function interpolated_path = interpolate_path(path, num_points)
    % Interpolate path with a given number of points
    interpolated_path = [];
    for i = 1:size(path, 1)-1
        % Compute the step size for each dimension
        step_size = (path(i+1, :) - path(i, :)) / (num_points + 1);
        % Generate interpolated points
        interpolated_segment = path(i, :) + (1:num_points).' * step_size;
        interpolated_path = [interpolated_path; interpolated_segment];
    end
end
